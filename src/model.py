...
class NetworkLIF:
    def __init__(self, dt=0.5, V_thr=-55, E_L=-65, tau_m=10.0, g_L=10.0, t_refr=2.0,
        ...
        if not isinstance(w_ff, np.ndarray):
            ...
            raise Exception(f"The shape of the input connectivity matrix should be ({n_neurons}, {n_inputs}), but it is {w_ff.shape}")
    ...
    def _time_step(self, V, I, refr):
        mu = 0.6  
        sigma = 0.4  
        noise = mu * self.dt + sigma * np.sqrt(self.dt) * np.random.randn(len(V))

        dV = (-(V - self.E_L) + I) / self.tau_m
        V += (dV + noise) * self.dt
