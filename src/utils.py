...
def jitter_spikes(spike_times, n_fired, jitter_window, sim_length):
    """
    Apply temporal jitter to spike times for `n_fired` neurons independently.

    Parameters:
        spike_times (list of lists): List of spike times for each fired neuron.
        n_fired (int): Number of neurons that fired.
        jitter_window (float): Maximum time (in ms) for jittering in both directions.
        sim_length (float): Length of the simulation time (in ms).

    Returns:
        np.array: 2D array with jittered spike times, shape (n_fired, len(spike_times[0])).
    """
    # Initialize an array for the jittered spike times
    jittered_spikes = np.zeros((n_fired, len(spike_times)))

    # Generate random jitter for each spike time in the pattern
    jitter = np.random.uniform(-jitter_window, jitter_window, size=(n_fired, len(spike_times)))

    # For each neuron, jitter its spike times independently
    for i in range(n_fired):

        # Apply jitter to those spike times
        jittered_times = np.array(spike_times) + jitter[i]

        # Clip jittered spike times to stay within bounds [0, sim_length]
        jittered_times = np.clip(jittered_times, 0, sim_length)

        # Update the jittered spike times for this neuron
        jittered_spikes[i] = jittered_times
...
def generate_lgn_inputs_sync(n_neurons, n_sources, pattern, input_rate, length, dt=1.0, jitter_window=3):
    spike_trains = np.zeros((n_neurons, length), dtype=np.int32)

    # Number of synchronous spike events
    n_events = int(input_rate * n_sources * length * dt / 1000.0)

    # Generate shared, irregular spike times
    spike_times = np.sort(np.random.choice(np.arange(1, length), size=n_events, replace=False))

    spike_times_j = jitter_spikes(spike_times, len(pattern), jitter_window, length)
    spike_times_j = np.round(spike_times_j).astype(int)
    spike_times_j = np.clip(spike_times_j, 0, length - 1)

    for i, neuron in enumerate(pattern):
        spike_trains[neuron, spike_times_j[i]] = 1
